...
> **说明**：这份文档是 Bytro 项目的单一真相来源。每次修改代码前，请先阅读与你任务相关的章节。修改完成后，如有设计决策变化，请同步更新本文档。
...
Bytro 是一个 **AI Native 代码开发工作台**可交互原型。
...
Bytro 使用 `zinc` 作为中性色主调，语义色使用 Tailwind 默认色阶：
...
defaultSize="28%"  minSize="12%"  maxSize="50%"

// WorkspaceArea Panel
defaultSize="55%"  minSize="20%"
```
...
```
横向 PanelGroup：autoSaveId="mochi-main"
纵向 PanelGroup：autoSaveId="workspace-vertical"
```
...
**职责**：根组件，持有顶层状态，组装三列 PanelGroup。

**状态**：

```ts
activeTaskId: string                  // 当前选中的 Task
showSidePanel: boolean                // WorkspaceArea 右侧 Outline 面板
showBottomPanel: boolean              // WorkspaceArea 底部 Terminal 面板
settingsTrigger: number               // 每次 +1 触发 WorkspaceArea 打开 Settings 面板
viewChangesTrigger: number            // 每次 +1 触发 WorkspaceArea 打开 Diff 面板
memoryTrigger: number                 // 每次 +1 触发 WorkspaceArea 打开 Memory 面板
...
```ts
// 通过 ImperativePanelHandle ref 控制
const taskRailPanelRef = useRef<ImperativePanelHandle>(null);
...
```tsx
// editor-main：无 bottom 时 defaultSize=100%，有 bottom 时 defaultSize=70%
<Panel defaultSize={showBottomPanel ? "70%" : "100%"} minSize="25%" id="editor-main" order={1}>
// editor-bottom：仅 showBottomPanel 时渲染
<Panel defaultSize="30%" minSize="10%" maxSize="60%" id="editor-bottom" order={2}>
```
...
```tsx
// ✅ 父容器用 min-h-0，不在 PanelGroup 上加 h-full
<div className="flex-1 flex flex-col min-h-0">
  <PanelGroup orientation="vertical" autoSaveId="workspace-vertical">
    <Panel id="editor-main" order={1} defaultSize="70%" minSize="25%">
    ...
    {showBottomPanel && (
      <Panel id="editor-bottom" order={2} defaultSize="30%" minSize="10%" maxSize="60%">
        {/* 内容 */}
      </Panel>
    )}
  </PanelGroup>
...
1. **每个 Panel 必须有 `id` + `order`**，尤其是条件渲染的 Panel
2. **`defaultSize` 必须与实际渲染的 Panel 数量匹配**：只有一个 Panel 时 `defaultSize="100%"`
3. **`PanelGroup` 上禁止 `className="h-full"`**（破坏 vertical 方向尺寸计算）
4. **水平 Group 中的 Panel 需要 `style={{ height: '100%' }}`**（覆盖 v4 默认的 `height: auto`）
5. `collapsible` 的 Panel 需同时设置 `collapsedSize="0%"` + `onCollapse` + `onExpand` 回调
6. 使用 `ImperativePanelHandle` ref 进行命令式折叠/展开
7. **所有 size props 使用百分比字符串格式**（`"17%"` 而非 `{17}`）

### PanelResizeHandle 样式模板

```tsx
// 横向
<PanelResizeHandle className="group w-[3px] bg-zinc-900 hover:bg-blue-600/40 transition-colors cursor-col-resize flex items-center justify-center">
  <div className="h-8 w-px bg-zinc-700 group-hover:bg-blue-500 rounded-full transition-colors" />
</PanelResizeHandle>

// 纵向
<PanelResizeHandle className="group h-1 bg-zinc-900 hover:bg-zinc-700 transition-colors cursor-row-resize flex items-center justify-center">
...
### A. PanelGroup 上加 `h-full`

```tsx
// ❌ 错误 — 破坏 vertical 方向高度计算
<PanelGroup className="h-full" orientation="vertical">
...
// ✅ 始终提供 id + order
{showPanel && <Panel id="my-panel" order={2} defaultSize="30%">...</Panel>}
```
...
Bytro 目前使用 **Trigger Counter Pattern** 实现从父组件（App.tsx）到深层子组件（WorkspaceArea 内部）的命令式操作，避免了 ref 透传。

```ts
// 1. App.tsx 持有 trigger state
const [memoryTrigger, setMemoryTrigger] = useState(0);

// 2. 传递给目标组件
<WorkspaceArea openMemoryTrigger={memoryTrigger} />

// 3. 任意位置触发（如 TaskRail → App → WorkspaceArea）
<TaskRail onOpenMemory={() => setMemoryTrigger(n => n + 1)} />

// 4. WorkspaceArea 内 useEffect 响应
useEffect(() => {
  if (!openMemoryTrigger) return;  // 0 时不触发（初始状态）
  // 打开或 focus memory 面板
}, [openMemoryTrigger]);
```

**扩展新的跨组件触发**：
1. App.tsx 增加 `const [xTrigger, setXTrigger] = useState(0)`
2. 将 `xTrigger` 作为 prop 传给目标组件
3. 目标组件 `useEffect(() => { if (!xTrigger) return; ... }, [xTrigger])`
4. 在需要触发的地方 `setXTrigger(n => n + 1)`
