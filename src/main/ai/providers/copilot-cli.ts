...
const COPILOT_META: ProviderMeta = {
  ...
  permissionFlags: {
    manual: [],
    autoEdit: ['--allow-all-tools', '--allow-all-paths'],
    plan: [],
    fullAuto: ['--allow-all-tools', '--allow-all-paths'],
...
export class CopilotProvider extends BaseCLIProvider {
  ...
  private copilotSessions = new Map<string, CopilotSessionEntry>()

  protected buildStreamJsonArgs(_config: SessionConfig, _resume: boolean): string[] {
    // Copilot uses per-turn spawn like OpenCode, not persistent stream-json stdin.
  ...
  override async endSession(sessionId: string): Promise<void> {
    const entry = this.copilotSessions.get(sessionId)
    if (entry) {
  ...
  override sendMessage(sessionId: string, content: string): void {
    ...
    // Aligned with Slock: copilot --output-format json --allow-all-tools --allow-all-paths -p <prompt> --model <model>
    const args = [
      '--output-format', 'json',
      '--allow-all-tools',
      '--allow-all-paths',
    ...
    child.on('error', (error) => {
      entry.activeChild = null
      this.handleTerminalFailure(sessionId, entry, error.message || 'Copilot process failed to start')
      this.copilotSessions.delete(sessionId)
    })
    ...
    child.on('exit', (code, signal) => {
      ...
      if (!entry.doneEmitted) {
        ...
        if (cleanExit) {
          ...
          const reason = entry.stderr.trim() || `Copilot process exited unexpectedly (${signal || code || 'unknown'})`
  ...
  private handleEvent(sessionId: string, entry: CopilotSessionEntry, event: any): void {
    if (event.type === 'done') {
  ...
  private handleTerminalFailure(sessionId: string, entry: CopilotSessionEntry, message: string): void {
